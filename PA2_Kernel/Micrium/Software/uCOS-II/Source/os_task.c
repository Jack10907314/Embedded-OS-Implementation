...
#if OS_TASK_CHANGE_PRIO_EN > 0u
INT8U  Jack_OSTaskChangePrio(INT8U  oldprio,
    INT8U  newprio)
{
#if (OS_EVENT_EN)
    OS_EVENT* pevent;
...
INT8U  OSTaskCreateExt (void   (*task)(void *p_arg),
                        ...
                        INT32U*   aperiodic_deadline) //¦bOSTaskCreateExt¤º·s¼WÅÜ¼Æ arrival¡Bexecution¡Bperiod
{
    if(id != OS_TASK_IDLE_ID)
        printf("id:%d  %p\n", id, aperiodic_arrival);
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, pbos, id, stk_size, pext, opt, arrival, execution, period, aperiodic, aperiodic_serversize, aperiodic_arrival, aperiodic_execution, aperiodic_deadline); //¦bOS_TCBInit¤º·s¼WÅÜ¼Æ arrival¡Bexecution¡Bperiod
        if (err == OS_ERR_NONE) {
            OS_TRACE_TASK_CREATE(OSTCBPrioTbl[prio]);
...
#if OS_TASK_CREATE_EXT_EN > 0u
INT8U  Jack_OSTaskCreateExt(void   (*task)(void* p_arg),
    void* p_arg,
    OS_STK* ptos,
    INT8U    prio,
    INT16U   id,
    ...
    INT32U* aperiodic_arrival,
    INT32U* aperiodic_execution,
    INT32U* aperiodic_deadline) //在 Jack_OSTaskCreateExt 新增 OS_TCB 所需要的變數
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB*)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        psp = OSTaskStkInit(task, p_arg, ptos, opt);           /* Initialize the task's stack          */
        //在 Jack_OS_TCBInit 新增 OS_TCB 所需要的變數
        err = Jack_OS_TCBInit(prio, psp, pbos, id, stk_size, pext, opt, arrival, execution, period,
            aperiodic, aperiodic_serversize, aperiodic_arrival, aperiodic_execution, aperiodic_deadline);
        if (err == OS_ERR_NONE) {
            OS_TRACE_TASK_CREATE(OSTCBPrioTbl[prio]);
            if (OSRunning == OS_TRUE) {                        /* Find HPT if multitasking has started */
                OS_Sched();
