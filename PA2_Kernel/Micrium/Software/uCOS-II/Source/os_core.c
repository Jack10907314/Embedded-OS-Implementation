    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                //在task快要完成時 先切換到下個deadline時間 這樣就能去判斷下個要換哪個task執行
                if (OSTimeGet() - OSTCBCur->begin_ready_time == OSTCBCur->response)
                {
                    OSTCBCur->deadline = (OSTCBCur->job_id + 2) * OSTCBCur->period + OSTCBCur->arrival;
                }
                OS_SchedNew();
                //加這行if判斷是為了迴避當有task已經快要完成時被其他優先權高的task給搶占
                //迴避掉這次切換讓原本的task先做完
                if (OSTimeGet() - OSTCBCur->begin_ready_time != OSTCBCur->response) 
                {
                    ...
                    if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                        ...
                        OSTCBCur->OSTCBCtxSwCtr++; //OSTCBCtxSwCtr加一
                       
                        //OSIntExit是在timetick中斷後會做，低優先權task會被高優先權task搶佔，所以在這裡print搶佔
                        //假設是idle task被搶占，print的方式會不一樣
                        if (OSTCBCur->OSTCBPrio == OS_TASK_IDLE_PRIO)
                            printf("%d\t Preemption\t task(%d)  \t\t task(%d)(%d)\n", OSTimeGet(),
                                OS_TASK_IDLE_PRIO,
                                OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_id);
                        else
                            printf("%d\t Preemption\t task(%d)(%d)\t\t task(%d)(%d)\n", OSTimeGet(),
                                OSTCBCur->OSTCBId, OSTCBCur->job_id,
                                OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_id);
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        //作業要print所有task的linked list 
        //mytcb先取OSTCBList的OS_TCB
        //之後mytcb = mytcb->OSTCBNext;一直等於下一個link的OS_TCB
        //就能print出所有的task的link list
        OS_TCB* mytcb = OSTCBList;
        while (mytcb != (OS_TCB*)0)
        {
            if(mytcb->OSTCBId == OS_TASK_IDLE_ID)
                printf("%d\t %p\t %p\t %p\t\n", mytcb->OSTCBPrio, mytcb->OSTCBPrev, mytcb, mytcb->OSTCBNext);
            else
                printf("%d\t %p\t %p\t %p\t\n", mytcb->OSTCBId, mytcb->OSTCBPrev, mytcb, mytcb->OSTCBNext);
            mytcb = mytcb->OSTCBNext;
        }
        //-----------------------------------------
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        //這段是加在OSTimeTick內
        //從ReadyTable得到目前正在Ready的task
        //把現在正在Ready但卻沒有在執行的task的反應時間加一
        for (INT8U y = 0; y < 8; y++) //jack
        {
            for (INT8U x = 0; x < 8; x++)
            {

                if ((OSRdyTbl[y] & (INT8U)1 << x) == (INT8U)1 << x) //確認是有有Ready
                {
                    INT8U prio = (y << 3u) + x;
                    if (OSTCBPrioTbl[prio] != OSTCBCur && prio != OS_TASK_IDLE_PRIO)
                    {
                        if (OSTimeGet() > OSTCBPrioTbl[prio]->arrival)
                        {
        ...
        //while查詢所有linked list的task
        OS_TCB* mytcb = OSTCBPrioTbl[OS_TASK_IDLE_PRIO]->OSTCBPrev;
        while (mytcb != (OS_TCB*)0)
        {
            //週期工作的miss deadline處理
            if (mytcb->aperiodic == 0)
            {
                //假設反應時間 大於週期 表示說task已經missdeadline了
                //這裡有兩種miss deadline處理方式
                if (OSTimeGet() - OSTCBCur->begin_ready_time != OSTCBCur->response)
                {
                    //第一種: while 直接卡住
                    /*if (OSTimeGet() > mytcb->deadline)
                    {
                        printf("%d\t MissDeadline\t Task(%d)(%d)\t\t -----------\n", OSTimeGet() - 1,
                            mytcb->OSTCBId, mytcb->job_id);
                        while (1) {}
                    }*/
                
                    //第二種: 放棄這次工作 直接切換下個週期工作
                    if (OSTimeGet() == mytcb->deadline)
                    {
                        printf("%d\t MissDeadline\t Task(%d)(%d)\t\t -----------\n", OSTimeGet(),
                            mytcb->OSTCBId, mytcb->job_id);
                        mytcb->begin_ready_time = OSTimeGet();
                        mytcb->response = mytcb->execution;
                        mytcb->OSTCBCtxSwCtr = 0;

                        //完成次數+1
                        mytcb->job_id += 1;

                        mytcb->deadline = (OSTCBCur->job_id + 1) * OSTCBCur->period + OSTCBCur->arrival;
                    }
                }
            }
            //非週期工作的miss deadline處理
            else if(mytcb->job_id < mytcb->aperiodic)
            {
                if (OSTimeGet() > mytcb->deadline)
                {
                    printf("%d\t Aperiodic job(%d) misses deadline.\n", OSTimeGet(), mytcb->job_id);
                    while (1) {}
                }
            }

            //當非週期工作真正到達要工作時
            if (mytcb->aperiodic > 0 && OSTimeGet() == mytcb->arrival)
            {
                mytcb->begin_ready_time = OSTimeGet();  //begin_ready_time是task轉變為Ready的時間點
                mytcb->response = mytcb->aperiodic_execution[mytcb->job_id]; //反應時間等於執行時間
                mytcb->OSTCBCtxSwCtr = 0; //context switch次數設為0
            }

            //查看非週期工作 時間已經是設定的到達時間 查看是否前面還有工作顯示相對應字串
            if (mytcb->aperiodic_count < mytcb->aperiodic)
            {
                if (OSTimeGet() == mytcb->aperiodic_arrival[mytcb->aperiodic_count])
                {
                    //計算前次工作的deadline
                    INT32U max = mytcb->aperiodic_arrival[mytcb->aperiodic_count];
                    INT32U previous_deadline = mytcb->aperiodic_execution[mytcb->aperiodic_count - 1];
                    previous_deadline = (INT32U)((float)previous_deadline / mytcb->aperiodic_serversize);
                    previous_deadline = mytcb->aperiodic_arrival[mytcb->aperiodic_count - 1] + previous_deadline;
                    if (max < previous_deadline)
                        max = previous_deadline;
                    INT32U arrival = max;

                    INT32U execution = mytcb->aperiodic_execution[mytcb->aperiodic_count];
                    INT32U a = mytcb->aperiodic_execution[mytcb->aperiodic_count];
                    INT32U period = (INT32U)((float)a / mytcb->aperiodic_serversize);
                    INT32U deadline = arrival + period;

                    //前次的工作還的deadline還沒過 顯示已到達但不工作
                    if (mytcb->aperiodic_count != 0 && mytcb->aperiodic_arrival[mytcb->aperiodic_count] < previous_deadline)
                    {
                    ...
                    mytcb->aperiodic_count += 1;
                }
            }
            mytcb = mytcb->OSTCBPrev;
        }
            ...
            if (ptcb->OSTCBDly != 0u) {                    /* No, Delayed or waiting for event with TO     */
                ...
                if (ptcb->OSTCBDly == 0u) {                /* Check for timeout                            */
                    ...
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                        ...
                        {
                            //這段是加在OSTimeTick內
                            //當某個task轉變為Ready的時間 設定變數數值
                            ptcb->begin_ready_time = OSTimeGet();  //begin_ready_time是task轉變為Ready的時間點
                            ptcb->response = ptcb->execution; //反應時間等於執行時間
                            ptcb->OSTCBCtxSwCtr = 0; //context switch次數設為0
                            ptcb->deadline = (ptcb->job_id + 1) * ptcb->period + ptcb->arrival; //jack
                        }
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if (OSTimeGet() > OSTCBCur->arrival) 
                {
                    //週期工作的處理方式
                    if (OSTCBCur->aperiodic == 0)
                    {
                        OSTCBCur->OSTCBCtxSwCtr++; //OSTCBCtxSwCtr加一

                        //task完成之後會執行OS_Sched所以在這裡print完成
                        //假設是task完成後切入idle task，print的方式會不一樣
                        if (OSTCBHighRdy->OSTCBPrio == OS_TASK_IDLE_PRIO)
                            printf("%d\t Completion\t task(%d)(%d)\t\t task(%d)  \t\t %d\t\t\t %d\n", OSTimeGet(),
                                OSTCBCur->OSTCBId, OSTCBCur->job_id,
                                OS_TASK_IDLE_PRIO,
                                OSTimeGet() - OSTCBCur->begin_ready_time, OSTCBCur->OSTCBCtxSwCtr);
                        else
                        ...
                        //task完成所以將變數初始化
                        OSTCBCur->begin_ready_time = OSTimeGet();
                        OSTCBCur->response = OSTCBCur->execution;
                        OSTCBCur->OSTCBCtxSwCtr = 0;

                        //完成次數+1
                        OSTCBCur->job_id += 1;

                        OSTCBCur->deadline = (OSTCBCur->job_id + 1) * OSTCBCur->period + OSTCBCur->arrival;
                    }
                    //非週期工作的處理方式
                    else if (OSTCBCur->aperiodic > 0 && OSTCBCur->job_id < OSTCBCur->aperiodic)
                    {
                        OSTCBCur->OSTCBCtxSwCtr++; //OSTCBCtxSwCtr加一

                        printf("%d\t Aperiodic job(%d) is finished.\n", OSTimeGet(), OSTCBCur->job_id);
                        printf("%d\t Completion\t task(%d)(%d)\t\t task(%d)(%d)\t\t %d\t\t\t %d\n", OSTimeGet(),
                            OSTCBCur->OSTCBId, OSTCBCur->job_id,
                            OSTCBHighRdy->OSTCBId, OSTCBHighRdy->job_id,
                            OSTimeGet() - OSTCBCur->begin_ready_time, OSTCBCur->OSTCBCtxSwCtr);
                        ...
                        OSTCBCur->job_id += 1;
                        if (OSTCBCur->job_id < OSTCBCur->aperiodic) //在設定的工作數量內
                        {
                            OSTCBCur->begin_ready_time = OSTCBCur->aperiodic_arrival[OSTCBCur->job_id]; //初始化在OS_TCB內新增的變數
                            OSTCBCur->response = OSTCBCur->aperiodic_execution[OSTCBCur->job_id];
                            OSTCBCur->OSTCBCtxSwCtr = 0;

                            //計算上個工作的deadline時間 並比較 此次預設的arrival跟上次的deadline哪個比較大 設定task真正要arrival的時間
                            INT32U max = OSTCBCur->aperiodic_arrival[OSTCBCur->job_id];
                            INT32U previous_deadline = OSTCBCur->aperiodic_execution[OSTCBCur->job_id-1];
                            previous_deadline = (INT32U)((float)previous_deadline / OSTCBCur->aperiodic_serversize);
                            previous_deadline = OSTCBCur->arrival + previous_deadline;
                            if (max < previous_deadline)
                                max = previous_deadline;
                            OSTCBCur->arrival = max;

                            OSTCBCur->execution = OSTCBCur->aperiodic_execution[OSTCBCur->job_id];
                            INT32U a = OSTCBCur->aperiodic_execution[OSTCBCur->job_id];
                            OSTCBCur->period = (INT32U)((float)a / OSTCBCur->aperiodic_serversize);
                            OSTCBCur->deadline = OSTCBCur->arrival + OSTCBCur->period;
...
static  void  OS_SchedNew (void)
{
    ...
    //從 OS_SchedNew 來實現EDF功能
    //比較所有task的deadline來決定要切換到哪個task
    //查看是否有除了idle task以外的task在ready
    OS_TCB* PrioHighTCB = OSTCBPrioTbl[OS_TASK_IDLE_PRIO];
    OS_TCB* mytcb = OSTCBPrioTbl[OS_TASK_IDLE_PRIO];
    while (mytcb != (OS_TCB*)0)
    {
        
        if ((OSRdyTbl[mytcb->OSTCBY] & (INT8U)1 << mytcb->OSTCBX) == (INT8U)1 << mytcb->OSTCBX) //確認是有有Ready
        {
            //只計算週期工作 以及 非週期工作在已設定的工作數量內
            if(mytcb->aperiodic == 0 || (mytcb->job_id < mytcb->aperiodic && OSTimeGet() >= mytcb->arrival))
                PrioHighTCB = mytcb;
        }
        
        mytcb = mytcb->OSTCBPrev;
    }
    //假設沒有其他工作 將工作切換到idle task
    if (PrioHighTCB == OSTCBPrioTbl[OS_TASK_IDLE_PRIO])
    {
        OSPrioHighRdy = OS_TASK_IDLE_PRIO;
    }
    else
    {
        mytcb = OSTCBPrioTbl[OS_TASK_IDLE_PRIO]->OSTCBPrev;
        while (mytcb != (OS_TCB*)0)
        {
            if ((OSRdyTbl[mytcb->OSTCBY] & (INT8U)1 << mytcb->OSTCBX) == (INT8U)1 << mytcb->OSTCBX) //確認是有有Ready
            {   //只計算週期工作 以及 非週期工作在已設定的工作數量內
                if (mytcb->aperiodic == 0 || mytcb->job_id < mytcb->aperiodic)
                {
                    //比較哪個task deadline最小
                    if (mytcb->deadline < PrioHighTCB->deadline) 
                    {
                        PrioHighTCB = mytcb;
                    }
                    //deadline相同的話 取id比較小的task
                    else if (mytcb->deadline == PrioHighTCB->deadline && mytcb->OSTCBId < PrioHighTCB->OSTCBId)
                    {
                        PrioHighTCB = mytcb;
                    }
                }
        ...
        OSPrioHighRdy = PrioHighTCB->OSTCBPrio;
    }
    ...
    /*INT8U count = OSTaskCtr - 1;

    OS_TCB** array_tcb;
    array_tcb = (OS_TCB**)malloc(count * (sizeof(OS_TCB*)));//動態申請記憶體
    memset(array_tcb, (OS_TCB*)0, count);//初始化，每個元素都為零
    ...
    for (INT8U i = 0; i < count; ++i) {
        for (INT8U j = 0; j < i; ++j) {
            if (array_tcb[j]->deadline > array_tcb[i]->deadline) {
                OS_TCB* temp = array_tcb[j];
                array_tcb[j] = array_tcb[i];
                array_tcb[i] = temp;
            }
            else if (array_tcb[j]->deadline == array_tcb[i]->deadline && array_tcb[j]->OSTCBId > array_tcb[i]->OSTCBId)
            {
                OS_TCB* temp = array_tcb[j];
                array_tcb[j] = array_tcb[i];
                array_tcb[i] = temp;
    ...
    for (INT8U i = 0; i < count; ++i)
    {
        //printf("task%d : deadline = %d\n", array_tcb[i]->OSTCBId, array_tcb[i]->deadline);
        Jack_OSTaskChangePrio(array_tcb[i]->OSTCBPrio, i + 1 + count);
    }
    for (INT8U i = 0; i < count; ++i)
    {
        //printf("task%d : deadline = %d\n", array_tcb[i]->OSTCBId, array_tcb[i]->deadline);
        Jack_OSTaskChangePrio(array_tcb[i]->OSTCBPrio, i + 1);
    }


    for (INT8U i = 0; i < count; ++i)
    {
        printf("task%d : priority = %d\n", array_tcb[i]->OSTCBId, array_tcb[i]->OSTCBPrio);
    }
    //OSTaskChangePrio(INT8U  oldprio, INT8U  newprio);

    free(array_tcb);//釋放記憶體，malloc和free一定要記得成組使用，不然會導致程式吃記憶體
    */
    

    /*if (OSPrioHighRdy != OSPrioCur)  //目前最高優先權task不是現在工作的task
    {
        if(OSPrioCur == 0)
            //與結果相同的格式
            printf("%d\t ********\t task(%d)\n", OSTimeGet(), OSPrioHighRdy);
        else
            //顯示現在timetick、目前task、切換到的下個task
...
INT8U  OS_TCBInit (INT8U    prio,
                   ...
                   INT32U   period,
                   INT32U   aperiodic,
                   float   aperiodic_serversize,
                   INT32U*   aperiodic_arrival[],
                   INT32U*   aperiodic_execution[],
                   INT32U*   aperiodic_deadline[]) //在OS_TCBInit內新增變數 arrival、execution、period
{
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->aperiodic = aperiodic;
        ptcb->aperiodic_serversize = aperiodic_serversize;
        ptcb->aperiodic_arrival = aperiodic_arrival;
        ptcb->aperiodic_execution = aperiodic_execution;
        ptcb->aperiodic_deadline = aperiodic_deadline;
        if (aperiodic == 0)
        {
            ptcb->begin_ready_time = arrival; //初始化在OS_TCB內新增的變數
            ptcb->response = execution;
            ptcb->arrival = arrival;
            ptcb->execution = execution;
            ptcb->period = period;
            ptcb->job_id = 0;
            ptcb->deadline = arrival + period;
        }
        else if (aperiodic == 1)
        {
            /*ptcb->begin_ready_time = aperiodic_arrival[0]; //初始化在OS_TCB內新增的變數
            ptcb->response = aperiodic_execution[0];
            ptcb->arrival = aperiodic_arrival[0];
            ptcb->execution = aperiodic_execution[0];
            ptcb->job_id = 0;
            INT32U a = aperiodic_execution[0];
            ptcb->period = (INT32U)((float)a / aperiodic_serversize);;
            ptcb->deadline = arrival + ptcb->period;*/
        }
        ...
        //OS_TCBInit會初始化task的OS_TCB所需要的變數
        //所以在這個地方print作業所需要的數值
        if (ptcb->OSTCBId == OS_TASK_IDLE_ID) //假如是idle_task的話就顯示優先權
        {
            printf("Task[  %d] created, TCB Address %p\n", ptcb->OSTCBPrio, ptcb);
...
INT8U  Jack_OS_TCBInit(INT8U    prio,
    OS_STK* ptos,
    OS_STK* pbos,
    INT16U   id,
    INT32U   stk_size,
    ...
    INT32U* aperiodic_arrival,
    INT32U* aperiodic_execution,
    INT32U* aperiodic_deadline) //在 Jack_OS_TCBInit 新增 OS_TCB 所需要的變數
{
    OS_TCB* ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
    ...
    if (ptcb != (OS_TCB*)0) {
        ...
        ptcb->aperiodic_execution = aperiodic_execution;
        ptcb->aperiodic_deadline = aperiodic_deadline;
        if (aperiodic == 0) //初始化 週期工作的變數
        {
            ptcb->begin_ready_time = arrival;
            ptcb->response = execution;
            ptcb->arrival = arrival;
            ptcb->execution = execution;
            ptcb->period = period;
        ...
        else if (aperiodic > 0) //初始化 非週期工作的變數
        {
            ptcb->begin_ready_time = aperiodic_arrival[0];
            ptcb->response = aperiodic_execution[0];
            ptcb->arrival = aperiodic_arrival[0];
            ptcb->execution = aperiodic_execution[0];
            ptcb->job_id = 0;
            //計算 非週期工作 此次的task的週期時間
            INT32U a = aperiodic_execution[0];
            ptcb->period = (INT32U)((float)a / aperiodic_serversize);
            ptcb->deadline = aperiodic_arrival[0] + ptcb->period; //EDF的deadline 就是 到達時間加週期時間
            ptcb->OSTCBDly = aperiodic_arrival[0];
        }
