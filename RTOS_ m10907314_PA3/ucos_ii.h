...
typedef struct os_event {
    ...
    INT8U    id;//紀錄resource的id  ex:R1的ID為1、R2的ID為2
...
typedef struct os_tcb {
    ...
    INT32U begin_ready_time; //紀錄task變為ready的時間點
    INT32U response; //反應時間
    INT32U arrival; //到達時間
    INT32U execution; //執行時間
    INT32U period; //週期
    INT32U job_id; //工作次數
    INT32U deadline; //最後期限
...
OS_EXT  OS_EVENT         *Jack_event_Tbl[OS_LOWEST_PRIO + 1u];    //新增一個Mutex event的table
#endif
