        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSTimeGet() - OSTCBCur->begin_ready_time != OSTCBCur->response) 
                {
                    ...
                    INT8U sw = 1; //紀錄是否要context switch

                    //遍歷所有Jack_event_Tbl表
                    //查看是否有當前task使用的資源的優先權比OSTCBHighRdy還高
                    for (int i = 0; i < 64; i++)
                    {
                        if (Jack_event_Tbl[i] != (OS_EVENT*)0)
                        {
                            if ((Jack_event_Tbl[i]->OSEventCnt & 0x00FF) == OSPrioCur)
                            {
#ifdef NPCS
                                //排程為NPCS時執行這段
                                //只要有使用任一資源就不做context switch
                                if ((Jack_event_Tbl[i]->OSEventCnt >> 8) != 0x00)
                                {
                                    sw = 0;
...
#ifdef CPP
                                //排程為CPP時執行這段
                                //只要當前task使用的資源的優先權比OSTCBHighRdy還高就不做context switch
                                if ((Jack_event_Tbl[i]->OSEventCnt >> 8) <= OSPrioHighRdy)
                                {
                                    sw = 0;
                                    OSPrioHighRdy = OSPrioCur;
                                    OSTCBHighRdy = OSTCBCur;
...
#ifdef CPP
        printf("Tick\tEvent\t\t\tPrio_Inheritance\n");
#endif // CPP
        ...
        for (INT8U y = 0; y < 8; y++) //jack
        {
            for (INT8U x = 0; x < 8; x++)
            {
                ...
                if ((OSRdyTbl[y] & (INT8U)1 << x) == (INT8U)1 << x) //確認是有有Ready
                {
                    ...
                    {
                        if (OSTCBPrioTbl[prio] != OSTCBCur && OSTimeGet() > OSTCBPrioTbl[prio]->arrival)
                        {
                            OSTCBPrioTbl[prio]->response += 1;

                            //假設反應時間 大於週期 表示說task已經missdeadline了
                            if (OSTCBPrioTbl[prio]->response > OSTCBPrioTbl[prio]->period)
                            {
                        ...
                        //只要是面前正在執行的task，runtime加1
                        if (OSTCBPrioTbl[prio] == OSTCBCur)
                        {
                            OSTCBCur->runtime += 1;
                        }
                    }
        ...
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (sw == 1) //是否做context switch
            {
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                    ...
                    if (OSPrioHighRdy == OS_TASK_IDLE_PRIO)
                        printf("%d\tTask %d\n", OSTimeGet(), OS_TASK_IDLE_PRIO);
                    else if (OSTimeGet() >= OSTCBHighRdy->arrival)
                        printf("%d\tTask %d\n", OSTimeGet(), OSTCBHighRdy->OSTCBId);
...
INT8U  OS_TCBInit (INT8U    prio,
                   ...
                   INT32U   arrival,
                   INT32U   execution,
                   INT32U   period) //在OS_TCBInit內新增變數 arrival、execution、period
{
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
