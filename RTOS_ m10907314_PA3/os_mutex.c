...
OS_EVENT*   Jack_OSMutexCreate(INT8U   prio,
                               INT8U  *perr,
                               INT8U   id)
{
    OS_EVENT* pevent;
    ...
    pevent->OSEventPtr = (void*)0;                       /* No task owning the mutex                 */
    pevent->id = id; 
    Jack_event_Tbl[id] = pevent; //將mutex的記憶體位置放入Jack_event_Tbl中
#if OS_EVENT_NAME_EN > 0u
    pevent->OSEventName = (INT8U*)(void*)"?";
#endif
    OS_EventWaitListInit(pevent);
...
#ifdef CPP
        //排程為CPP時
        //遍歷所有Jack_event_Tbl表，查看當前task使用的資源的最高優先權
        INT8U highestPrio = OSPrioCur;
        for (int i = 0; i < 64; i++)
        {
            if (Jack_event_Tbl[i] != (OS_EVENT*)0 && Jack_event_Tbl[i] != pevent)  //(OS_EVENT*)0 和 當前event 不判斷
            {
                if ((Jack_event_Tbl[i]->OSEventCnt & 0x00FF) == OSPrioCur)
                {
                    if ((Jack_event_Tbl[i]->OSEventCnt >> 8) <= highestPrio)
                    {
                        highestPrio = (Jack_event_Tbl[i]->OSEventCnt >> 8);
                    }
                }
            }
        }
        if( (pevent->OSEventCnt >> 8) < highestPrio)  //當前event的優先權比較大時
            printf("%d\tTask %d get R%d\t\t%d>>%d\n", OSTimeGet(), OSTCBCur->OSTCBId, pevent->id, highestPrio, pevent->OSEventCnt >> 8);
        else if(highestPrio < OSPrioCur)
