...
INT8U  OSTaskCreateExt (void   (*task)(void *p_arg),
                        ...
                        INT32U   arrival,
                        INT32U   execution,
                        INT32U   period) //在OSTaskCreateExt內新增變數 arrival、execution、period
{
    OS_STK     *psp;
    INT8U       err;
#if OS_CRITICAL_METHOD == 3u                 /* Allocate storage for CPU status register               */
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, pbos, id, stk_size, pext, opt, arrival, execution, period); //在OS_TCBInit內新增變數 arrival、execution、period
        if (err == OS_ERR_NONE) {
            OS_TRACE_TASK_CREATE(OSTCBPrioTbl[prio]);
