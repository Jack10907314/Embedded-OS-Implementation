...
void  OSTimeDly (INT32U ticks)
{
    ...
    if (ticks > 0u && OSTCBCur->arrival < OSTimeGet())
    {
        //task初始化參數
    ...
    //當發生直行 OSTimeDly(0) 的情況表示反應時間剛好是週期時間
    //task已經完成這次週期的工作要做下個週期的工作
    if (OSTCBCur->period == OSTCBCur->response)
    {
        //print這次週期的工作完成 輪到下個週期的工作開始
        /*printf("%d\t Completion\t task(%d)(%d)\t\t task(%d)(%d)\t\t %d\t\t\t %d\n", OSTimeGet(),
            OSTCBCur->OSTCBId, OSTCBCur->job_id,
            OSTCBCur->OSTCBId, OSTCBCur->job_id + 1,
